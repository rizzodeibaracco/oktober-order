...
const LABELS = {
  q_bionda_05:"Bionda 0,5L",q_bionda_1:"Bionda 1L",
  q_rossa_05:"Rossa 0,5L",q_rossa_1:"Rossa 1L",
  q_nf_05:"Non filtrata 0,5L",q_nf_1:"Non filtrata 1L",
  q_cola:"Coca-Cola",q_nat:"Acqua naturale",q_frizz:"Acqua frizzante"
};
// Firebase init
...
const db=firebase.database();const ORDERS=db.ref("orders");
...
function renderDrinkList(){
  ...
  for(let k in PRICE){
    ...
    div.innerHTML=`<div class="item-title">${LABELS[k]} (${PRICE[k]} €)</div>
    <div class="qty-controls">
      <button type="button" class="btn round accent" data-target="${k}" data-delta="-1">-</button>
      <span id="${k}" class="qty-display">0</span>
      <button type="button" class="btn round primary" data-target="${k}" data-delta="1">+</button>
    </div>`;
    dl.appendChild(div);
  }
}
...
document.addEventListener("click",e=>{if(e.target.dataset&&e.target.dataset.delta){const id=e.target.dataset.target;let v=+document.getElementById(id).textContent;v=Math.max(0,v+parseInt(e.target.dataset.delta));document.getElementById(id).textContent=v;updateTotal();}});
function clearForm(){for(let k in PRICE)document.getElementById(k).textContent="0";updateTotal();document.getElementById("waiter").value="";document.getElementById("table").value="";}
...
document.getElementById("order-form").addEventListener("submit",e=>{
  ...
  const items=[];for(let k in q){if(q[k]>0)items.push(`${LABELS[k]} × ${q[k]}`);}
...
function renderOrders(snap){
  ...
  let tot=0,active=0,ready=0,delivered=0;const filter=document.getElementById("status-filter").value;
  Object.entries(data).sort((a,b)=>a[1].createdAt-b[1].createdAt).forEach(([id,o])=>{
    tot+=o.total||0;
    if(o.status==="pending")active++;else if(o.status==="ready")ready++;else if(o.status==="delivered")delivered++;
...
document.getElementById("status-filter").addEventListener("change",()=>{ORDERS.once("value").then(renderOrders);});
document.getElementById("orders-list").addEventListener("click",e=>{
  if(e.target.dataset&&e.target.dataset.act){
    ...
    else if(act==="delete"){if(confirm("Eliminare ordine?"))ORDERS.child(id).remove();}
  }
});
